...
impl<'a, T: BoolReal> Kernel12<'a, T> {
    pub fn op(&self, p1: usize, q2: usize) -> Option<(i32, Vector3<T>)> {
        let mut x12 = 0;
        ...
        Some((x12, Vector3::new(xzyy[0], xzyy[2], xzyy[1])))
    }
}

pub fn intersect12<T: BoolReal>(
    mp: &Manifold<T>,
    mq: &Manifold<T>,
    p1q2: &mut Vec<[usize; 2]>,
    expand: T,
    fwd: bool,
) -> (Vec<i32>, Vec<Vector3<T>>) {
