...
fn main() {
    ...
    println!("Done. {} fixture files generated.", fs::read_dir(fixture_dir).unwrap().count());
}
...
fn generate_transform_fixtures(dir: &Path) {
    ...
    write_fixture(&dir.join("cube_translated.bincode"), "cube_translated", &translated);
...
fn generate_boolean_fixtures(dir: &Path) {
    ...
    write_fixture(&dir.join("cube_intersect_cube.bincode"), "cube_intersect_cube", &inter);
    ...
    write_fixture(&dir.join("cyl_sphere_add.bincode"), "cyl_sphere_add", &cyl_sph);

    let torus = generate_torus::<f64>(1., 0.1, 30, 30).unwrap();
    let result = compute_boolean(&cyl_sph, &torus, OpType::Subtract).unwrap();
    write_fixture(&dir.join("cyl_sphere_sub_torus.bincode"), "cyl_sphere_sub_torus", &result);
}
...
fn generate_extrusion_fixtures(dir: &Path) {
    fn make_hollow_square() -> MultiPolygon<f64> {
        ...
        println!("make_hollow_square exterior: {:?}", canon.0[0].exterior().coords().collect::<Vec<_>>());
        if !canon.0[0].interiors().is_empty() {
            println!("make_hollow_square interior: {:?}", canon.0[0].interiors()[0].coords().collect::<Vec<_>>());
        }
        canon
    }
    ...
    write_fixture(&dir.join("extrude_square.bincode"), "extrude_square", &extrude_square);

    let hollow = make_hollow_square();
    let extrude_hollow = hollow.extrude(1.0, 1, 0.0, Vector2::new(1.0, 1.0)).unwrap();
    write_fixture(&dir.join("extrude_hollow_square.bincode"), "extrude_hollow_square", &extrude_hollow);

    let extrude_twisted = hollow.extrude(1.0, 20, PI, Vector2::new(1.0, 1.0)).unwrap();
    write_fixture(&dir.join("extrude_twisted.bincode"), "extrude_twisted", &extrude_twisted);

    let extrude_tapered = hollow.extrude(1.0, 50, PI, Vector2::new(0.0, 0.0)).unwrap();
    write_fixture(&dir.join("extrude_tapered.bincode"), "extrude_tapered", &extrude_tapered);
}
...
fn generate_revolution_fixtures(dir: &Path) {
    ...
    write_fixture(&dir.join("revolve_square.bincode"), "revolve_square", &revolve);

    let revolve_half = square_with_bite.revolve(5, PI * 0.5).unwrap();
    write_fixture(&dir.join("revolve_half.bincode"), "revolve_half", &revolve_half);
}
...
fn generate_projection_slice_fixtures(dir: &Path) {
    ...
    write_fixture(&dir.join("cube_slice_025.bincode"), "cube_slice_025", &slice_025);

    let slice_m025 = cube.slice(-0.25).unwrap();
    write_fixture(&dir.join("cube_slice_minus025.bincode"), "cube_slice_minus025", &slice_m025);

    let cyl = generate_cylinder::<f64>(1., 1., 30, 10).unwrap();
    let cyl_proj = cyl.project_xy().unwrap();
    write_fixture(&dir.join("cylinder_project.bincode"), "cylinder_project", &cyl_proj);

    let sphere = generate_uv_sphere::<f64>(30, 30).unwrap();
    let sphere_proj = sphere.project_xy().unwrap();
    write_fixture(&dir.join("uv_sphere_project.bincode"), "uv_sphere_project", &sphere_proj);

    let ico = generate_icosphere::<f64>(1).unwrap();
    let ico_proj = ico.project_xy().unwrap();
    write_fixture(&dir.join("icosphere_project.bincode"), "icosphere_project", &ico_proj);

    let torus = generate_torus::<f64>(1., 0.1, 30, 30).unwrap();
    let torus_proj = torus.project_xy().unwrap();
    write_fixture(&dir.join("torus_project.bincode"), "torus_project", &torus_proj);
    ...
    write_fixture(&dir.join("extrude_square_project.bincode"), "extrude_square_project", &proj);
    ...
    write_fixture(&dir.join("extrude_hollow_project.bincode"), "extrude_hollow_project", &proj);
    ...
    write_fixture(&dir.join("revolve_square_project.bincode"), "revolve_square_project", &proj);
}
...
fn generate_composition_fixtures(dir: &Path) {
    ...
    write_fixture(&dir.join("compose_two_cubes.bincode"), "compose_two_cubes", &composed);
}
