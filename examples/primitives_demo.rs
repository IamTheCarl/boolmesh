...
fn setup(
    ...
    for (fid, hs) in res.hs.chunks(3).enumerate() {
        let p0 = res.ps[hs[0].tail.0];
        let p1 = res.ps[hs[1].tail.0];
        let p2 = res.ps[hs[2].tail.0];
